...
from lara.core.io import jsonl_path_lock, read_json, write_json_atomic
from lara.schema import CaseRunRef, RunConfig
from lara.tracing import EventStore

from .context import LaraContext
...
class BashTaskObservations:
    """Persist Lara audit work; Pygent alone owns command execution and results."""
    ...
    async def prepare(self, context: LaraContext) -> None:
        await self._finish_thread(self._prepare, context)

    def _prepare(self, context: LaraContext) -> None:
    ...
    def track(self, context: LaraContext, result: ToolResult,
    ...
    def _start(self, path: Path) -> None:
        ...
        with independent_execution():
            observer = asyncio.create_task(
                self._observe(path), name=f"lara-bash-observation:{path.stem}",
            )
