...
def _to_pygent_message(message: dict[str, Any]) -> PygentMessage | None:
    ...
    # Historical Lara sessions predate that field, so normalize only the
...
def _initial_lara_context(
    *,
    context: LaraContext,
    history: list[dict[str, Any]],
    checkpoint: object | None = None,
) -> tuple[LaraContext, bool]:
    ...
    if checkpoint is not None:
        restored = cast(
            LaraContext,
            context_from_dict(checkpoint, registry=LARA_CONTEXT_CODECS),
        )
        return (
            replace(
                context,
...
def _latest_user_input_hash(history: list[dict[str, Any]]) -> str | None:
    for message in reversed(history):
        ...
        is_automation = (
            message.get("kind") == "lara.automation.trigger"
