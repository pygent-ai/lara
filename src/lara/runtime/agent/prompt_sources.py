...
def _render_system_identity_prompt(ctx: PromptRenderContext) -> str:
    return "\n".join(
        [
            "# Identity",
            "",
            "You are Lara, an interactive coding agent for software engineering work.",
...
def _render_system_path_policy_prompt(ctx: PromptRenderContext) -> str:
    project_lara_root = _ctx_project_lara_root(ctx)
    user_lara_root = _ctx_user_lara_root(ctx)
    return "\n".join(
        [
            "# Lara Path Layout",
            "",
            f"- Workspace root: {ctx.workspace_root}",
            f"- Per-project Lara state: {project_lara_root}",
            f"- Lara home: {user_lara_root}",
            ...
            "- Per-project Lara state resources belong to this workspace. Lara home resources are reusable across projects.",
...
def _render_system_lara_control_plane_prompt(ctx: PromptRenderContext) -> str:
    return "\n".join(
        [
            "# Lara Control Plane",
            "",
            "- For capabilities managed by Lara, use the corresponding `uv run lara ...` CLI.",
            "- Treat an unqualified scheduled or recurring task request as a Lara automation.",
            ...
            "- Do not claim that a Lara resource was created or changed until the command succeeds and its list or show command confirms the persisted state.",
        ]
    )
...
def _render_available_tools_prompt(ctx: PromptRenderContext) -> str:
    ...
    lines = [
        ...
        "",
        f"Workspace root: {ctx.workspace_root}",
        "Default search excludes: .git, .hg, .lara, .mypy_cache, .nox, .pytest_cache, .ruff_cache, .svn, .tox, .venv, __pycache__, node_modules, venv.",
...
def _prompt_render_context_payload(ctx: PromptRenderContext) -> dict[str, Any]:
    return {
        ...
        "user_lara_root": str(_ctx_user_lara_root(ctx)),
        "project_lara_root": str(_ctx_project_lara_root(ctx)),
...
def _ctx_project_lara_root(ctx: PromptRenderContext) -> Path:
    return (
        (
            ctx.project_lara_root
            or project_lara_root(ctx.workspace_root, _ctx_user_lara_root(ctx))
        )
        .expanduser()
        .resolve()
    )
...
def _ctx_user_lara_root(ctx: PromptRenderContext) -> Path:
    return (ctx.user_lara_root or Path.home() / ".lara").expanduser().resolve()


def _ctx_project_skills_dir(ctx: PromptRenderContext) -> Path:
    return (
        (ctx.project_skills_dir or _ctx_project_lara_root(ctx) / "skills")
        .expanduser()
        .resolve()
    )
...
def _ctx_user_skills_dir(ctx: PromptRenderContext) -> Path:
    return (
        (ctx.user_skills_dir or _ctx_user_lara_root(ctx) / "skills")
        .expanduser()
        .resolve()
    )
