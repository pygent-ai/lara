...
from lara.cli.credentials import register_credentials_parser
from lara.cli.automations import register_automation_parser
from lara.cli.sessions import register_session_parser
...
from lara.workflows import execute_case_run
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"lara: error: {exc}", file=sys.stderr)
        return 2
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="lara", description="Agent self-optimization harness CLI"
    )
    parser.add_argument(
        "--workspace-root",
        default=None,
        help="Workspace root. Defaults to cwd or LARA_WORKSPACE_ROOT.",
    )
    parser.add_argument(
...
def _regression_run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    manifest = Path(manager.config.lara_root) / "regression.json"
