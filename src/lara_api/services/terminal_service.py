...
from lara_api.models.responses import TerminalCommandResponse
from lara_api.services.workspace_service import workspace_root_for_scope
...
class PowerShellSession:
    ...
    def execute(self, command: str) -> TerminalCommandResponse:
        with self._lock:
            ...
            exit_marker = f"__LARA_EXIT_{token}__"
            cwd_marker = f"__LARA_CWD_{token}__"
            ...
            script = (
                ...
                "$global:LASTEXITCODE=0;$laraOk=$true;"
                "try { Invoke-Expression $c; $laraOk=$? } "
                "catch { Write-Output ('ERROR: ' + $_.Exception.Message); $laraOk=$false };"
                "$laraCode=if($laraOk){[int]$LASTEXITCODE}else{1};"
                f"Write-Output ('{exit_marker}'+$laraCode);"
