...
from lara.runtime.agent.common import _initial_lara_context
from lara.runtime.context import LARA_CONTEXT_CODEC, LARA_CONTEXT_CODECS, LaraContext
from lara.runtime.file_effect_models import DeferredFileEffectJob


def test_lara_context_keeps_model_projection_and_native_commits_distinct() -> None:
    ...
    assert isinstance(projected, LaraContext)
    assert projected.committed_messages == (current,)
    assert [item["content"] for item in projected.history] == ["current"]


def test_lara_context_codec_round_trips_all_portable_state() -> None:
    context = LaraContext(
        session_id="session-1",
        case_id="chat",
        case_run_id="run-1",
    ...
    registry = ContextCodecRegistry((LARA_CONTEXT_CODEC,))

    restored = context_from_dict(
        context_to_dict(context, registry=registry), registry=registry
    )

    assert restored == context
    assert isinstance(restored, LaraContext)


def test_lara_context_owns_and_drains_pending_file_effects() -> None:
...
def test_native_context_restore_uses_the_checkpoint_projection() -> None:
    ...
    restored, compacted = _initial_lara_context(
        context=context,
        history=history,
...
def test_eternal_bootstrap_restores_history_instead_of_empty_checkpoint() -> None:
    context = LaraContext(eternal_memory_enabled=True)
    checkpoint = context_to_dict(context, registry=LARA_CONTEXT_CODECS)
    history = [
        message_to_dict(UserMessage(content="keep this requirement")),
...
def test_uncovered_memory_suffix_keeps_tool_pairs_and_all_later_turns() -> None:
    ...
    from lara.runtime.service import _uncovered_conversation_messages

    messages = [
        UserMessage(content="covered turn"),
        ...
        ToolMessage(
            results=(
                ToolResult(
                    call_id="read-1", name="read", output="data", status="succeeded"
                ),
            )
        ),
        AIMessage(content="inspection finished"),
        UserMessage(content="another requirement"),
    ...
    assert _uncovered_conversation_messages(history, len(history)) == tuple(
        messages[-2:]
    )
