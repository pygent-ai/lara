...
from lara.config import clear_user_model_config, load_run_config, replace_user_model_config
from lara.config.yaml_subset import dump_yaml_subset, parse_yaml_subset
...
def test_yaml_subset_dump_round_trips_native_model_values() -> None:
    data = {
        "lara_root": r"C:\Users\agent's data\.lara",
...
def test_unknown_model_request_field_is_rejected(tmp_path: Path) -> None:
    ...
    with patch("lara.config.loader.Path.home", return_value=home), pytest.raises(ValueError, match=r"model_request contains unknown fields: model_name"):
...
def test_native_model_fields_are_validated_by_pygent(tmp_path: Path) -> None:
    ...
    with patch("lara.config.loader.Path.home", return_value=home), pytest.raises(ValueError, match="missing model fields: connection"):
...
def test_runtime_and_context_settings_are_loaded(tmp_path: Path) -> None:
    ...
    assert config.runtime_capacity.coordinator_path == str((Path(config.lara_root) / ".state/capacity.sqlite3").resolve())


def test_missing_agent_alias_is_rejected(tmp_path: Path) -> None:
    home = write_user_config(tmp_path, NATIVE_CONFIG)
    with patch("lara.config.loader.Path.home", return_value=home), pytest.raises(ValueError, match="not configured"):
...
def test_user_model_config_is_shared_across_project_workspaces(tmp_path: Path) -> None:
    ...
    (home / ".lara" / "credentials.env").write_text("USER_MODEL_KEY=user-secret\n", encoding="utf-8")
...
def test_project_config_is_not_a_configuration_source(tmp_path: Path) -> None:
    ...
    workspace.mkdir()
    (workspace / "lara.yaml").write_text(NATIVE_CONFIG, encoding="utf-8")
    with patch("lara.config.loader.Path.home", return_value=home):
