...
def test_history_timing_uses_checkpoint_ownership_across_restart(tmp_path):
    ...
    for status in ["passed", "error", "skipped"]:
        ...
        for role in ["user", "assistant"]:
            manager.append_history_checkpoint(
                run,
                turn_id="turn",
                checkpoint_id=f"{run.case_run_id}:{role}",
    ...
    assert all(
        "run_timing" not in row for row in restarted.load(session.session_id).history
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("execution.deadline_exceeded", "error"),
    ],
)
async def test_terminal_stream_waits_for_durable_timing_and_replays_it(
    ...
    turn = ManagedSessionTurn(
        ...
        command=TurnCommand(session_id=ref.session_id, message="hi"),
        run_ref=run_ref,
    ...
    replay = [
        event
        async for event in AttachedExecutionRun(Handle(), run_ref, manager).events(
            after=None
        )
    ]
