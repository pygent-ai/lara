...
```powershell
uv run lara session run --new --message "检查当前项目"
uv run lara --agent dev session run --new --message "运行测试并总结失败"
uv run lara session run <session_id> --message "继续处理"
```
...
```powershell
uv run lara session chat --new
uv run lara session chat <session_id>
```

交互模式直接订阅 Pygent execution journal：

- `model.text.delta` 输出正文；
- `lara.approval.requested` 触发终端审批；
- execution 完成、失败或取消由 Pygent Runtime 管理。

模型通过用户级 `~/.lara/config.yaml` 中的
...
```powershell
# 列出当前作用域的会话
uv run lara session list

# 新建会话并在独立 worker 中执行
uv run lara session start --source-session <current_session_id> --message "检查失败用例" --submission-id task-001

# 给正在工作的已有会话异步发送 Agent 消息
uv run lara session send <session_id> --source-session <current_session_id> --message "补充检查类型错误" --submission-id task-002

# 查询或等待后台 operation / Agent message
uv run lara session status <operation_id>
uv run lara session wait <operation_id>
uv run lara session status <message_id>
uv run lara session wait <message_id>
```
...
`SessionCollaborationStore` 投递，CLI 不直接创建或关闭 `LaraRuntimeService`。
