...
`lara automation` manages durable local scheduled tasks. In a source checkout,
invoke it as `uv run lara automation`. The desktop/API process
executes queued work; CLI commands read and write the shared
`~/.lara/automations-v1.sqlite3` database.

Create a standalone one-time task:

```powershell
lara --workspace-root E:\Projects\example automation create `
  --name "检查构建" `
  --prompt "运行测试并总结失败原因" `
...
Scheduled execution requires Lara Desktop or `lara-api` to be running. A periodic
task missed while Lara was stopped is caught up once at the next startup.
